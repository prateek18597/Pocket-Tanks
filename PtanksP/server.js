...
function attack(tx,power,angle)
{
	...
	var txx=0;
	var p=0.1;
	// console.log(terr);// power/=10;
	for(var i=tx;i<1000;i+=1)
	{
		
		ty=power*Math.sin(angle*3.14/180)*p-5*p*p;
		
		txx=Math.floor(power*Math.cos(angle*3.14/180)*p);
		console.log(tank1y);
		console.log(terr[txx+tank1x]);
		if(tank1y-ty>=terr[txx+tank1x])
		{
			console.log("Break.");
			break;
		}

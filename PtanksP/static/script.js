...
function tanks()
{
	...
	var bitmap = new createjs.Bitmap("tank.png");
	bitmap.x=tank1x-10;
	bitmap.x=tank1x-15;
...
socket.on('attackR',function(data){
		...
		g1.beginFill("black");
		g1.drawCircle(tank1x,tank1y-10,5);
		...
		{
			attackC.push(attackC[attackC.length-2]+1);
			attackC.push(attackC[attackC.length-1]-0.1);	
		}
		console.log(attackC);
		// stage.addChild(attackBall);
		// stage.update();
		createjs.Tween.get(attackBall).to({guide:{ path:attackC }},attackC/);
		stage.addChild(attackBall);
		stage.update();
		// stage.removeChild(attackBall);
		// for(var i=0;i<atatackC.length;i+=1)
		// {
